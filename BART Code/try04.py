...
def prune(tree):
    ...
    for c in pNode.fpointer:
        if c in leaf: leaf.remove(c)
    b = len(leaf)+1
    ...
    print(str(mi)+' prune '+str(t)+': '+pNode.tag+'; r='+str(r.round(4)))
...
def grow(tree):    
    ...
    r = rLike*rTransit*rStruct
    print(str(mi) + ' grow '+str(t)+': '+str((pid,x,s))+'; r='+str(r.round(4)))
    if random.uniform(0,1) < r:      
...
def genTree(node,sList=[]):    
    ...
    for split in sList:
        ...
        cid = array([pid,pid])*2+array([1,2])
        for i in range(2):
            if len(childDF[i])<=0: raise IndexError('no data in leaf')
...
def swap(tree):
    ...
    try:
        ...
        print(str(mi) + ' swap '+str(t)+': '+str(tags[0])+'; unswappable')
        return tree
    rTransit = 1
    rLike = get_likelihood(tree.R,sub.leaves(),sub1.leaves())
    rStruct = get_struct(sub.all_nodes_itr(),sub1.all_nodes_itr())
    r = rLike*rTransit*rStruct
    print(str(mi) + ' swap '+str(t)+': '+str(tags[0])+'; r='+str(r.round(4)))
    if random.uniform(0,1) < r: 
        if pid > 0:
...
def change(tree):    
    ...
    rStruct = get_struct(sub.all_nodes_itr(),sub1.all_nodes_itr())
    r = rLike*rTransit*rStruct
    print(str(mi) + ' change '+str(t)+': '+str(tags[0])+'; r='+str(r.round(4)))
    if random.uniform(0,1) < r:    
        tree1.w2 = tree.w2        
        tree1.show()
        tree1.R = tree.R
...
def get_likelihood(R,leaf,leaf1):
