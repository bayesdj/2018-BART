...
sst = (y-ymu)@(y-ymu)
ig1 = v/2; 
ig2 = minimize(lambda x:abs(gamma.cdf(lamda,ig1,scale=x)-q),x0=1/ig1)['x'][0]
ig2 = minimize(lambda x:abs(gamma.cdf(lamda,ig1,scale=x)-q),x0=ig2)['x'][0]
ig2 = 1/ig2
a = ig1+n0*0.5-1
#gg = gamma.cdf(lamda,ig1,scale=ig2)
#g = stats.invgamma.cdf(var,ig1,scale=1/ig2)
...
for i in range(m):
    for l in trees[i].leaves():
        idx = l.data.index
        MM.loc[idx,i] = l.data.iloc[:,0].mean()/m
#%%  
Like = zeros(T)
Rmse = zeros(T)
# = zeros(T) 
