...
def prune(tree):
    leaf0 = tree.leaves()
    leaf = filter(lambda n:len(tree.siblings(n.identifier)[0].fpointer)==0,
                   leaf0) # only pairs of terminal nodes with no children
    leaf2 = list(set([node.bpointer for node in leaf])) 
    w2 = len(leaf2)
    pid = random.choice(leaf2)
    pNode = tree[pid]
    leaf = list(filter(lambda n: (n.identifier not in pNode.fpointer)
                    and len(n.xvar)>0, leaf0)) # leaf for grow
    b = len(leaf)+1
