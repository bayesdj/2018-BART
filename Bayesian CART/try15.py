...
def genTree(sList,node):
    ...
    if isinstance(node,pd.core.frame.DataFrame)
        tree = Tree()
        root = tree.create_node(tag='0',identifier=0,data=node)
...
def nidValid(tree,internalNodes):
...
def changeTree(tree):
    ...
    nidInternal = nidValid(tree,internalNodes)
    ...
    tag = tree[nid].tag.split('-')
    x0 = int(tag[1]); s0 = str2num(tag[2])
    choices = choiceDic[nid]
    choices[x0-1].remove(s0) # remove original split option
    choices2 = [i+1 for i in range(p-1) if len(choices[i])>0]
