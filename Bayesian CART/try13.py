...
def changeTree(tree):
    nInternal = [n for n in tree.all_nodes_itr() if n.var != None]
    # make sure obj data type's ancestor are not in selection
    nObj = (n for n in nInternal if DataTypes[n.var]=='O') 
    noChange = []
    for node in nObj:
        current = node
        while current.identifier != tree.root:
            ...
            if ancestor.var == node.var:
                noChange.append(ancestor.identifier)
                break
            current = ancestor
    nInternal = [n.identifier for n in 
                 filter(lambda n:n.identifier not in noChange, nInternal)]
    #nid = random.choice(nInternal)
    choices = [getChoice(tree,n) for n in nInternal]
    n_choices = map(lambda L: max([len(i) for i in L]),choices)
    g = {a:b for (a,b,c) in zip(nInternal,choices,n_choices) if c > 1}
    return g
