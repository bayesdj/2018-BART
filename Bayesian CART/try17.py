...
def genData(n=800,seed=None):
    ...
    y = array(list(map(f1,tuple(zip(x1,x2))))) + 1*np.random.normal(size=n)
...
def grow(tree):    
    ...
    pGrow = Prob[0]; pPrune = Prob[1]-pGrow
    rLike = sqrt(p1)*p2
    ...
    if random.uniform(0,1) < r:      
        ...
        node1 = tree.create_node(str(cid[0]),cid[0],parent=pid,data=childDF[0])
        node2 = tree.create_node(str(cid[1]),cid[1],parent=pid,data=childDF[1])        
...
def change(tree):    
    ...
    rTransit = len(choices1)*len(choices3)/(len(choices11)*len(choices31))
